...
void*
MapPointer(soff_entry* soffs, int soffs_len, DWORD in_ptr, int* section)
{
    int i;
...
/*
int
FindSectionID(IMAGE_OPTIONAL_HEADER* oh, DWORD address, DWORD size)
{
    int i;
...
int
FindSectionByRawData(LOADED_IMAGE* img, DWORD address)
{
    ULONG i;
...
void
ResizeArray(void** data, uint64_t* data_size, size_t sizeof_data)
{
    uint64_t new_size = (*data_size) > 0 ?
                        (*data_size) * 2 :
                        64;
    ...
    memset(((unsigned char*)new_data) + (*data_size * sizeof_data),
           0,
...
#define ResizeDepList(ptr_deptree, ptr_deptree_size) \
                ResizeArray((void**)ptr_deptree, ptr_deptree_size, sizeof(struct DepTreeElement*))
#define ResizeImportList(ptr_import_list, ptr_import_list_size) \
                ResizeArray((void**)ptr_import_list, ptr_import_list_size, sizeof(struct ImportTableItem))
#define ResizeStack(ptr_stack, ptr_stack_size) \
                ResizeArray((void**)ptr_stack, ptr_stack_size, sizeof(char*))

void
AddDep(struct DepTreeElement* parent, struct DepTreeElement* child)
{
...
AddImport(struct DepTreeElement *self)
{
...
int
FindDep(struct DepTreeElement* root, char* name, struct DepTreeElement** result)
{
    int ret = -1;
    ...
    for (i = 0; i < root->childs_len; i++) {
        if (stricmp(root->childs[i]->module, name) == 0) {
            ...
            return (root->childs[i]->flags & DEPTREE_UNRESOLVED) ?
                   1 :
                   0;
        }
    }
...
ProcessDep(BuildTreeConfig* cfg,
           soff_entry* soffs,
           int soffs_len,
           DWORD name,
           struct DepTreeElement* root,
           struct DepTreeElement* self,
           int deep)
{
...
FindExportForward(struct DepTreeElement* self,
                  char* dllname,
                  char* export_name,
                  DWORD export_ordinal)
{
    return NULL;
}
*/
...
int
ClearDepStatus(struct DepTreeElement* self, uint64_t flags)
{
    uint64_t i;
...
void
PushStack(char*** stack, uint64_t* stack_len, uint64_t* stack_size, char* name)
{
...
void
PopStack(char*** stack, uint64_t* stack_len, uint64_t* stack_size, char* name)
{
...
static uint64_t
thunk_data_u1_function(void* thunk_array, DWORD index, BuildTreeConfig* cfg)
{
...
static void*
opt_header_get_dd_entry(void* opt_header, DWORD entry_type, BuildTreeConfig* cfg)
{
    if (cfg->machineType == IMAGE_FILE_MACHINE_I386) {
...
static void
BuildDepTree32or64(LOADED_IMAGE* img,
                   BuildTreeConfig* cfg,
                   struct DepTreeElement* root,
                   struct DepTreeElement* self,
                   soff_entry* soffs,
...
BOOL
TryMapAndLoad(PCSTR name, PCSTR path, PLOADED_IMAGE loadedImage, int requiredMachineType)
{
    ...
    if (success &&
        requiredMachineType != -1 &&
        (int)loadedImage->FileHeader->FileHeader.Machine != requiredMachineType) {
...
int
BuildDepTree(BuildTreeConfig* cfg,
             char* name,
             struct DepTreeElement* root,
             struct DepTreeElement* self)
{
    ...
    for (i = 0; i < img->NumberOfSections; i++) {
        ...
        if (cfg->on_self) {
            ...
            soffs[i].off = img->MappedAddress + img->Sections[i].PointerToRawData - img->Sections[i].VirtualAddress;
        } else {
            soffs[i].off = NULL;
        }
    }
        ...
        if (self->imports[i].mapped == NULL &&
            ...
            for (j = 0; j < dll->exports_len; j++) {
                if ((self->imports[i].name != NULL &&
                     ...
                     strcmp(self->imports[i].name, dll->exports[j].name) == 0)
                    ||
                    (self->imports[i].ordinal > 0 &&
                     dll->exports[j].ordinal > 0 &&
